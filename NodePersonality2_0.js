...
export class NodePersonality2_0 {
  constructor(scene) {
    ...
    // Cached objects for performance (avoid per-frame allocations)
    this.tempVector = new THREE.Vector3();  // Reusable for quantum jitter
  ...
  update(deltaTime, worldTime) {
    ...
    if ((this.registry.frameCounter % 2) !== 0) return;
    ...
    for (const [nodeId, personalityState] of this.registry.nodePersonalities) {
      ...
      // Intensity-based update rate: higher intensity = more frequent updates
      const skipFrames = Math.max(1, 5 - personalityState.intensityLevel);
      if ((this.registry.frameCounter % skipFrames) !== 0) continue;
  ...
  updateQuantum(personalityState, deltaTime, worldTime) {
    ...
    // Store jitter (for optional rendering use) - use cached vector to avoid allocation
    this.tempVector.set(jitterX, jitterY, jitterZ);
    node.userData.personalityJitterOffset = this.tempVector.clone();
