param(
  ...
  [switch]$PreferStaged,
  [string]$ModelName = ""  # voliteľné: napr. "nemotron-3-nano" ak chceš skúsiť /model
)
...
function Build-ContextFile([string]$root, [string]$outDir, [string[]]$files, [bool]$gitMode, [switch]$preferStaged) {
  ...
  if ($gitMode) {
    ...
    if ($diff.Count -gt $maxLines) {
      $lines.AddRange($diff[0..($maxLines-1)])
      $lines.Add("")
...
try {
  if (-not (Get-Command cn -ErrorAction SilentlyContinue)) {
    throw "Chýba Continue CLI. Nainštaluj: npm i -g @continuedev/cli"
  }
