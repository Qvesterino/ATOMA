...
export class FXRuntime_v1 {
    ...
    update(delta) {
        ...
        try {
            for (const [name, fx] of Object.entries(this.fxSystems)) {
                if (!fx || !fx.update || typeof fx.update !== 'function') {
                    continue;  // Skip systems without update
                }

                try {
                    // CRITICAL FIX: Normalize narrativePatterns nodes before update
                    // narrativePatterns expects: (deltaTime, nodes, links, worldMetrics)
                    // Ensure nodes and links are ALWAYS arrays to prevent "not iterable" errors
                    if (name === 'narrativePatterns' && fx.update.length >= 2) {
                        ...
                        // VISUAL READINESS GATE: Only process visualReady nodes
                        ...
                        // SAFETY: Early return if no ready nodes to avoid fallback material mutations
                        if (nodes.length === 0) {
                            continue;  // Silent skip - do NOT log error, do NOT mutate
                        }
                        
                        // Call with normalized, validated, ready nodes only
                        fx.update(delta, nodes, links, this.game?.worldMetrics || {});
                    } else {
                        // Most systems take deltaTime as first parameter
                        fx.update(delta);
                    }
                } catch (err) {
                    // Log individual system failures but continue with others
                    console.warn(`[FXRuntime_v1] Update failed for ${name}:`, err.message);
                }
            }
        } catch (err) {
