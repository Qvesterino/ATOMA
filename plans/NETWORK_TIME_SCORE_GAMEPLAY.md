...
|------|-------------|--------|
| `VisualNetworkTimeElasticity_v1.js` | Visual-only time rewind effect when synergy > 0.85 for 5s | **Refactor target** — becomes gameplay score authority |
| `CoreMetricsHUD.js` | Has local `networkTimeCounter` that increments at 5 units/sec, freezes at synergy >= 0.85 | **Refactor target** — reads from score system instead |
...
```mermaid
flowchart TD
    A[MetricsRuntime computes networkSynergy] --> B{synergy >= 0.82?}
...
SCORE_CONFIG = {
  ...
  synergyThreshold: 0.82,   // same as MetricsRuntime global.synergy.high
...
- **Synergy threshold**: 0.82 (aligned with MetricsRuntime)
...
1. `MetricsRuntime` emits `global.synergy.high` when `networkSynergy >= 0.82`
...
5. **Synergy threshold**: 0.82 (aligned with MetricsRuntime `global.synergy.high`).
