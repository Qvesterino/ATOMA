...
function findFiles(dir, pattern) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      if (['Config.Msi', 'Windows', 'Program Files', 'Program Files (x86)', '$Recycle.Bin'].includes(entry.name)) continue;
...
class ReportGenerator {
  ...
  printConsole() {
    ...
    for (const r of this.reports) {
      ...
      console.log('+' + '-'.repeat(boxWidth) + '+');
      const checkLine = r.checks.map(c => `${c.name}:${c.passed ? 'OK' : 'FAIL'}`).join(' ');
  ...
  generateMarkdown() {
    ...
    lines.push('|------|----------|-------|---------|------|---------|--------|');
    for (const r of this.reports) {
      const checksStr = r.checks.map(c => `${c.name}:${c.passed ? 'OK' : 'FAIL'}`).join(' ');
    ...
    for (const r of this.reports) {
      if (r.riskLevel === 'MID' || r.riskLevel === 'HIGH') {
        ...
        for (const c of r.checks) {
          if (!c.passed) lines.push(`- **${c.name}:** FAIL — ${c.detail}`);
        }
        lines.push('');
      }
    }
