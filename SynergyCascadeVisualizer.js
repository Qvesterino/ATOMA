...
export class SynergyCascadeVisualizer {
  ...
  renderNodeSynergyRipple(event = {}) {
    ...
    if (tier === 'low') {
      if (!this._tryConsumeRippleBudget({
        ...
        anchor,
        sourceCooldownSeconds: Math.max(this.config.synergyRippleCooldownSeconds, 2.5),
        anchorCooldownSeconds: this.config.sharedLineRippleCooldownSeconds
      })) return;
  ...
  _getSharedRippleCooldownKey(anchor, options = {}) {
    ...
    if (explicitKey) {
      return `line:${String(explicitKey)}`;
    }
    ...
    return `line:${Math.round(resolved.x * 2)}:${Math.round(resolved.y * 2)}:${Math.round(resolved.z * 2)}`;
  }

  _canSpawnSharedLineRipple(anchor, options = {}) {
    const cooldownSeconds = Math.max(
      0,
      Number(options.cooldownSeconds ?? this.config.sharedLineRippleCooldownSeconds) || 0
    );
    if (cooldownSeconds <= 0) return true;
    const key = this._getSharedRippleCooldownKey(anchor, options);
    return this._canTriggerGlobalRipple(key, cooldownSeconds);
  }
  ...
  renderCascadeStart(event = {}) {
    ...
    if (link) {
      ...
      if (this.config.visualizations.rippleEffect && this._tryConsumeRippleBudget({
        ...
        this._spawnRippleCluster(anchor, Math.max(0.1, visualIntensity), band.rippleCount, {
          ...band,
          flatRipple: true,
          cooldownKey: linkId || cascadeId || 'cascade-start',
          cooldownSeconds: this.config.sharedLineRippleCooldownSeconds
        });
      }
    ...
    if (this.config.visualizations.rippleEffect) {
      if (this._tryConsumeRippleBudget({
        ...
        anchor,
        sourceCooldownSeconds: 3.5,
        anchorCooldownSeconds: this.config.sharedLineRippleCooldownSeconds
      })) {
        this.createRipple(anchor, Math.max(0.1, intensity), { colorTint: 0x4488aa, radiusScale: 0.7, opacityScale: 0.5, lifetimeScale: 0.7 });
      }
    }
    if (this.config.visualizations.burstParticles) {
  ...
  renderCascadeHop(event = {}) {
    ...
    if (link) {
      ...
      if (anchor && this.config.visualizations.rippleEffect && this._tryConsumeRippleBudget({
        ...
        this._spawnRippleCluster(anchor, Math.max(0.1, visualIntensity), band.rippleCount, {
          ...band,
          flatRipple: true,
          cooldownKey: linkId || cascadeId || 'cascade-hop',
          cooldownSeconds: this.config.sharedLineRippleCooldownSeconds
        });
      }
    ...
    if (anchor && this.config.visualizations.rippleEffect) {
      if (this._tryConsumeRippleBudget({
        ...
        this.createRipple(anchor, Math.max(0.09, intensity * 0.5), { colorTint: 0x4488aa, radiusScale: 0.7, opacityScale: 0.5, lifetimeScale: 0.7 });
      }
    }
    if (anchor && this.config.visualizations.burstParticles) {
  ...
  applyCascadeGlowEffect(link, history, propagation) {
    ...
    if (material.emissive) {
      const glowColor = this.config.cascadeColor.clone()
  ...
  spawnFlowParticles(propagation, cascade) {
    ...
    forward.normalize();
    const _upRef = Math.abs(forward.y) < 0.9 ? new THREE.Vector3(0, 1, 0) : new THREE.Vector3(1, 0, 0);
    const side = new THREE.Vector3().crossVectors(forward, _upRef).normalize();
  ...
  createRipple(position, intensity, options = {}) {
    ...
    if (options.colorTint && Number.isFinite(options.colorTint)) {
      rippleColor = rippleColor.clone().lerp(new THREE.Color(options.colorTint), 0.45);
    }
    const ripple = {
