 ...
 * 
 * Manages mid-run draft UI, crisis phase activation, and metric modifier application.
 */
...
export class DoctrineRuntime {
  constructor({ game = null, metricsRuntime = null, runIdentityDirector = null } = {}) {
    this._game = game;
  ...
  bind({ game, metricsRuntime, runIdentityDirector } = {}) {
  ...
  triggerMidrunDraft() {
    ...
    this._draftPhase = 'school_selection';
  ...
  selectSchool(schoolId) {
    const school = getSchoolOfThought(schoolId);
    if (!school) return false;
    
    this._state = applyDraftToState(this._state, { schoolId }, this._world);
    this._activeSchool = schoolId;
    
    if (this._draftPhase === 'school_selection') {
  ...
  applyModifiersToMetrics(rawMetrics = {}) {
    ...
    const applied = {
      ...
      corruption: rawMetrics.corruption * (modifiers.corruption.spread || 1),
  ...
  _renderSchoolSelection() {
    ...
    title.textContent = 'Choose Your School';
    
    const subtitle = document.createElement('div');
    subtitle.className = 'doctrine-overlay-subtitle';
    subtitle.textContent = 'This doctrine shapes how metrics behave for the rest of this run.';
    ...
    for (const school of schools) {
      if (!SCHOOL_OF_THOUGHT_DEFINITIONS[school.id]) continue;
      ...
      cardTitle.textContent = school.label;
      
      const cardDesc = document.createElement('div');
      cardDesc.className = 'doctrine-option-desc';
      cardDesc.textContent = school.description;
      
      const modTags = document.createElement('div');
      modTags.className = 'doctrine-option-mods';
      
      const mods = school.metricModifiers;
      if (mods.synergy?.scale && mods.synergy.scale > 1) {
        const tag = document.createElement('span');
        tag.className = 'doctrine-mod-tag synergy';
        tag.textContent = `Synergy ×${mods.synergy.scale}`;
        modTags.appendChild(tag);
      }
      if (mods.stability?.scale && mods.stability.scale > 1) {
        const tag = document.createElement('span');
        tag.className = 'doctrine-mod-tag stability';
        tag.textContent = `Stability ×${mods.stability.scale}`;
        modTags.appendChild(tag);
      }
      if (mods.loadPressure?.active) {
        const tag = document.createElement('span');
        tag.className = 'doctrine-mod-tag pressure';
        tag.textContent = 'Pressure Active';
        modTags.appendChild(tag);
      }
      ...
      card.addEventListener('click', () => this.selectSchool(school.id));
  ...
  _createOverlayDOM() {
    ...
    if (!document.getElementById('doctrine-overlay-styles')) {
      ...
      style.textContent = `
        ...
        .doctrine-mod-tag.synergy { background: rgba(0, 212, 255, 0.15); color: rgba(100, 240, 255, 0.9); }
        .doctrine-mod-tag.stability { background: rgba(0, 200, 120, 0.15); color: rgba(100, 240, 180, 0.9); }
        .doctrine-mod-tag.pressure { background: rgba(255, 100, 160, 0.15); color: rgba(255, 140, 180, 0.9); }
        .doctrine-skip-btn {
          display: block;
          margin: 18px auto 0;
